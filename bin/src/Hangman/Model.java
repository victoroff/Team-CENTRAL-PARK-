...
public class Model {
	...
	private boolean isLetterUsed = false;
	...
	public boolean isLetterUsed() { // added now
		return isLetterUsed;
	}

	public void setLetterUsed(boolean isLetterUssed) { // added now
		this.isLetterUsed = isLetterUssed;
	}

	public Model(String _word) {
		this.isLetterUsed = isLetterUsed(); // added now
	...
	public void setChoice(char choice) {
		...
		try {
			int indexToRemoveLetterFromAvailableCharacters = this.availableCharacters
					.indexOf(Character.toLowerCase(choice));
			this.availableCharacters
					.remove(indexToRemoveLetterFromAvailableCharacters);
		} catch (ArrayIndexOutOfBoundsException e) {
			this.setLetterUsed(true);
		}
		if (this.isLetterGuessed() == false && this.isLetterUsed == false) {
			this.setNumberOffWrongAnswers(this.getNumberOffWrongAnswers() + 1);
		}
		if (this.getNumberOffWrongAnswers() >= this
				.getNumberOffMaximalWrongAnswers()) {
